...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node_i in reverse_topo_order:
        ...
        grad_node_i=sum(node_to_output_grads_list[node_i])
        # save the gradient
        node_i.grad=grad_node_i
        if not node_i.is_leaf():
            for child_node, partial_grad in zip(node_i.inputs,node_i.op.gradient(grad_node_i,node_i)):
                if child_node not in node_to_output_grads_list:
                    node_to_output_grads_list[child_node] = [partial_grad]
                else:
                    node_to_output_grads_list[child_node].append(partial_grad)
    ### END YOUR SOLUTION
